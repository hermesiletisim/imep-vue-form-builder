<template>
    <div class="sidebar"
        :class="{ mirrored: isSidebarControlConfiguration, not_mirrored: !isSidebarControlConfiguration }">
        <!-- <span class="close"
              @click="close"
              v-html="$form.getIcon('close', '24px', '24px', '#000')">
        </span> -->
        <!--- For dynamic purpose --->
        <component v-if="component?.name == 'SidebarControlSelectList'" :is="component" :dataPackage="dynamicData"
            :formData="formData" :permissions="permissions" :runnerId="runnerId" @save="save"
            @saveAndClose="saveAndClose" @close="close" />
        <component v-if="component?.name != 'SidebarControlSelectList'" :is="component" :dataPackage="dynamicData"
...
export default {
    ...
    methods: {
        ...
        saveAndClose(specialData = {}) {
            ...
            if(this.component.name == 'SidebarControlSelectList'){
